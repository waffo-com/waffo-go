...
- **Webhook failure reason parsing**: `orderFailedReason`, `refundFailedReason`, and subscription `failedReason` now accept real callback payloads where the failure reason is delivered as a JSON-encoded string, while preserving map-like field access for existing Go callers.
- **Webhook failure callback coverage**: Added failed callback tests for payment, refund, and subscription webhook handling.

### Changed

- **Go E2E validation**: Refund E2E flows now use DANA, and sandbox-dependent branches now fail or skip explicitly instead of silently passing when a prerequisite state is unavailable.
